...
- **Property tests on a library** (`78502a0`)**, and it paid for itself on the first run.** 496 tests against
  ...
  - **The one thing hand written property tests could not do is shrink.** This project has had
    ...
    print the random string that broke and leave the reader to work out which character
    mattered. CsCheck reduces a failure to the smallest input that still fails and prints a
    seed. That is the whole purchase.
  - **The first run found a real defect.** `SnapshotJson.TryRead` threw a
    `NullReferenceException` on a damaged snapshot, shrunk to five characters removed at one
    place. `Snapshot` is a positional record, and **`required` does not reach constructor
    parameters** - so a file saying `"metadata": null` deserialises perfectly well and fails on
    the next line. A snapshot is kept for months and carried between machines, so "somebody
    edited it" and "the disk filled up mid-write" are ordinary. `bws snapshot diff` answered
    that with a stack trace instead of the sentence the method exists to produce.
  - **Generated by damaging a real snapshot rather than from nothing**, in the four ways a
    stored file actually goes wrong: it stops early, a piece falls out, a byte flips, something
    is inserted. Random text fails at the first character and proves only that the first
    character is checked.
  - **Mutation turned up something worth keeping:** removing the null check does not compile,
    because the nullable analysis then refuses the dereference two lines down. That half is
    held by the compiler, which is stronger than a test - so the mutation aims at the other
    half, that a refusal returns a reason and **no** snapshot rather than both.
  - **Two properties moved rather than being duplicated.** The two about arbitrary input went
    to the library; the two about ordering stayed hand written, because shrinking has nothing
    to offer a permutation of seven fixed members.
  - **Three more parsers now have the same property**, and two never had one: the launch path
    resolver, which takes whatever a service registered, and the snapshot reader. Both answer
    to nobody and both are read on somebody else's machine.
  - **The dependency register has a guard now.** `DeclaredReferenceGuards` read only project
    references until today, so a package could arrive in the core and nothing would say a word.
    It reads packages as well, with a per-project allow list - added on the day a claim was
    made out loud that a test-only package never reaches anything that ships.
  - **Licences read in the repositories, not on the package labels:** CsCheck's `LICENSE` is
    the full Apache-2.0 text and there is **no `NOTICE` file**, which is what decides whether
    §4(d) applies - it does not. §4 is about distribution, and this sits in a test project with
