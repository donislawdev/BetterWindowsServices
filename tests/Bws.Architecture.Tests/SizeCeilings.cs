...
/// <summary>
/// The six numbers the size ratchet holds, and the record of why each one stands where it does.
///
/// Split out of SizeRatchetGuards.cs on 2026-08-25, and the ratchet asked for it in the most
/// direct way available: that file stood on exactly 500 lines, so a two-line note explaining why
/// a ceiling was coming down made the file that counts long files into a long file. That has now
/// happened three times in one day, and twice the answer was to shorten the note - which is the
/// thing this guard exists to stop somebody doing to real code.
///
/// The seam is the one the file was already shaped around: about seven lines in ten here are the
/// HISTORY of a number rather than a test, and history is what grows every time a ceiling moves.
/// Keeping it beside the number rather than in a changelog is deliberate - the next session
/// reads it at the moment it is blocked by the number, which is the only moment it matters.
/// </summary>
internal static class SizeCeilings
{
    ...
    /// </summary>
    internal const int LongestTestFile = 714;
    ...
    /// </summary>
    internal const int ShippedMarkupFilesAllowedToBeLong = 1;
