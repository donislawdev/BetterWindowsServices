...
public sealed class SnapshotContractTests : IDisposable
{
    ...
    public void The_file_says_what_it_needs_to_be_compared_later()
    {
        ...
        // FOUR SINCE 2026-09-06, when `dependents` joined the entry document - who breaks if an
        // entry stops, which the plan has always asked the manager and nothing a person could read
        // ever held. The owner's decision, taken with the price named: a file written before that
        // day cannot be compared against one written after it.
        //
        // Two since 2026-08-25, when perUserRole joined, and three since 08-26, when the enumerated
        // values stopped being written in two conventions. This number is pinned here on purpose:
        // it is the only thing that turns "your file is from an older build" into a sentence its
        // owner can act on, and a change to it that nobody argued for is a change that silently
        // stops older snapshots from being readable.
        //
