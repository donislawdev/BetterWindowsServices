...
internal readonly record struct ListState
{
    ...
    /// </summary>
    /// <param name="partial">
    /// Whether some entries could not be judged - refused, or never read. The query engine counts
    /// both as one number, and either way "nothing matches" would be a claim about entries nobody
    /// could look at.
    /// </param>
    ...
    ///
    /// <b>AND "NOTHING THAT COULD BE READ" WHEN SOME ENTRIES COULD NOT BE JUDGED - the remainder of
        ...
        if (partial)
        {
            return scope == EntryScope.Everything
                ? Say(
                    ...
                    Texts.Of("gui.empty.nothingMatchedReadable"),
                    Texts.Of("gui.empty.nothingMatchedWayOut"))
                : Say(
                    ListFace.NothingMatched,
                    Texts.Of("gui.empty.nothingMatchedReadableHere"),
